...
const LostPetcard = ({ pet }) => {
  ...
  <div className="rounded-md p-2 bg-[#F5F5F5] space-y-2 text-sm group  hover:bg-[#C1C1C1]">
      <div className="relative">
      <img loading="lazy" src={pet.imageURL} alt={pet.name} className="w-full h-52 md:h-64 object-cover" />
      ...
      <h2 className="text-md md:text-2xl font-bold">{pet.name}</h2>
  ...
  <button className="bg-red-600 text-white px-1 md:px-3 font-bold py-1 rounded-md text-xs md:text-md w-full md:w-auto text-center">Reward: {pet.reward}</button>
</div>
